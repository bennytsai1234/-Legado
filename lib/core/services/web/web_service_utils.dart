  ...
  Future<dynamic> handlePostExtended(String path, String body) async {
    switch (path) {
      case '/saveBookProgress':
        ...
        if (book != null) {
          await bookDao.updateProgress(book.bookUrl, progress.durChapterIndex, progress.durChapterPos, progress.durChapterTitle ?? "");
          return "";
        }
        return null;
      case '/saveReplaceRule':
        final rule = ReplaceRule.fromJson(jsonDecode(body));
        await replaceDao.insertOrUpdate(rule);
        return "";
      case '/deleteReplaceRule':
        final rule = ReplaceRule.fromJson(jsonDecode(body));
        await replaceDao.delete(rule.id);
        return "";
      case '/saveRssSource':
        final source = RssSource.fromJson(jsonDecode(body));
        await rssDao.insertOrUpdate(source);
        return "";
      default:
        return null;
    }
