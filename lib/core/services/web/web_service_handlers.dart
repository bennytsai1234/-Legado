...
extension WebServiceHandlers on WebServiceBase {
  ...
  Future<dynamic> handleGet(String path, Map<String, String> params) async {
    switch (path) {
      ...
      case '/getBookshelf':
        final books = await bookDao.getBookshelf();
        return books.map((b) => b.toJson()).toList();
      case '/getChapterList':
        final url = params['url'];
        if (url == null) return null;
        var chapters = await chapterDao.getChapters(url);
  ...
  Future<dynamic> handlePost(String path, String body) async {
    switch (path) {
      ...
      case '/saveBookSource':
        ...
        await sourceDao.insertOrUpdate(source);
        return true;
      ...
      case '/deleteBookSources':
        ...
        await sourceDao.deleteSources(urls.cast<String>());
        return true;
      case '/saveBook':
