...
mixin WebDAVBackup on WebDAVBase {
  ...
  Future<bool> backup() async {
    ...
    try {
      ...
      _addJsonToZip(encoder, 'bookSource.json', (await BookSourceDao().getAll()).map((e) => e.toJson()).toList(), dir);
      _addJsonToZip(encoder, 'replaceRule.json', (await ReplaceRuleDao().getAll()).map((e) => e.toJson()).toList(), dir);
      _addJsonToZip(encoder, 'bookGroup.json', (await BookGroupDao().getAll()).map((e) => e.toJson()).toList(), dir);
      _addJsonToZip(encoder, 'bookmark.json', (await BookmarkDao().getAll()).map((e) => e.toJson()).toList(), dir);
      _addJsonToZip(encoder, 'readRecord.json', (await ReadRecordDao().getAll()).map((e) => e.toJson()).toList(), dir);
      _addJsonToZip(encoder, 'rssSource.json', (await RssSourceDao().getAll()).map((e) => e.toJson()).toList(), dir);
      _addJsonToZip(encoder, 'rssStar.json', (await RssStarDao().getAll()).map((e) => e.toJson()).toList(), dir);
      _addJsonToZip(encoder, 'dictRule.json', (await DictRuleDao().getAll()).map((e) => e.toJson()).toList(), dir);
      _addJsonToZip(encoder, 'httpTts.json', (await HttpTtsDao().getAll()).map((e) => e.toJson()).toList(), dir);
      _addJsonToZip(encoder, 'txtTocRule.json', (await TxtTocRuleDao().getAll()).map((e) => e.toJson()).toList(), dir);
