...
class _MangaReaderPageState extends State<MangaReaderPage> {
  ...
  void initState() {
    ...
    _init();
    _scrollCtrl.addListener(() { if (_readingMode != 1) setState(() => _currentPage = (_scrollCtrl.offset / 600).floor().clamp(0, _urls.length - 1)); });
  }

  @override
  void dispose() { _scrollTimer?.cancel(); _scrollCtrl.dispose(); _pageCtrl.dispose(); _transCtrl.dispose(); SystemChrome.setEnabledSystemUIMode(SystemUiMode.edgeToEdge); super.dispose(); }

  Future<void> _init() async {
    _chapters = await ChapterDao().getChapters(widget.book.bookUrl);
    _loadChapter(_currentIndex);
  }

  Future<void> _loadChapter(int i) async {
    final s = await BookSourceDao().getByUrl(widget.book.origin);
    if (s == null) return;
    setState(() { _isLoading = true; _currentIndex = i; });
    try {
  ...
  void _toggleAutoScroll() {
    if (_readingMode == 1) return;
    setState(() { _isAutoScrolling = !_isAutoScrolling; _showControls = false; });
    if (_isAutoScrolling) {
      ...
      GestureDetector(
        ...
        child: _isLoading ? const Center(child: CircularProgressIndicator(color: Colors.white)) : InteractiveViewer(
          ...
          child: _readingMode == 1 ? PageView.builder(controller: _pageCtrl, reverse: _isReverse, itemCount: _urls.length, onPageChanged: (i) => setState(() => _currentPage = i), itemBuilder: (c, i) => MangaImageView(imageUrl: _urls[i], readingMode: _readingMode))
                                   : ListView.builder(controller: _scrollCtrl, itemCount: _urls.length, padding: EdgeInsets.zero, itemBuilder: (c, i) => MangaImageView(imageUrl: _urls[i], readingMode: _readingMode)),
        ),
      ),
      if (!_showControls) Positioned(bottom: 4, left: 0, right: 0, child: _buildInfoBar()),
      AnimatedPositioned(duration: const Duration(milliseconds: 200), top: _showControls ? 0 : -100, left: 0, right: 0, child: MangaTopBar(bookName: widget.book.name, onBack: () => Navigator.pop(context), onShowToc: _showToc)),
      AnimatedPositioned(duration: const Duration(milliseconds: 200), bottom: _showControls ? 0 : -150, left: 0, right: 0, child: MangaBottomControls(readingMode: _readingMode, isAutoScrolling: _isAutoScrolling, isReverseDirection: _isReverse, brightness: _brightness, hasPrev: _currentIndex > 0, hasNext: _currentIndex < _chapters.length - 1, onModeChanged: (v) => setState(() => _readingMode = v), onToggleAutoScroll: _toggleAutoScroll, onToggleDirection: () => setState(() => _isReverse = !_isReverse), onBrightnessChanged: (v) => setState(() => _brightness = v), onPrevChapter: () => _loadChapter(_currentIndex - 1), onNextChapter: () => _loadChapter(_currentIndex + 1))),
      if (_brightness < 1.0) IgnorePointer(child: Container(color: Colors.black.withValues(alpha: 1.0 - _brightness))),
    ])));
  }

  Widget _buildInfoBar() => Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 2), color: Colors.black.withValues(alpha: 0.4), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('${_chapters.isNotEmpty ? _chapters[_currentIndex].title : ""} (${_currentPage + 1}/${_urls.length})', style: const TextStyle(color: Colors.white70, fontSize: 10)), Text(DateFormat('HH:mm').format(DateTime.now()), style: const TextStyle(color: Colors.white70, fontSize: 10))]));

  void _showToc() => showModalBottomSheet(context: context, backgroundColor: Colors.grey[900], builder: (ctx) => ListView.builder(itemCount: _chapters.length, itemBuilder: (c, i) => ListTile(title: Text(_chapters[i].title, style: TextStyle(color: i == _currentIndex ? Colors.blue : Colors.white70)), onTap: () { Navigator.pop(ctx); _loadChapter(i); })));
}
