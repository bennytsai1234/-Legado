...
class _SourceManagerPageState extends State<SourceManagerPage> {
  ...
  Widget _buildSourceList(SourceManagerProvider p) {
    ...
    if (p.sources.isEmpty) return const Center(child: Text('暫無書源'));
    final bool canReorder = p.sortMode == 0 && !p.groupByDomain;
    return canReorder ? ReorderableListView.builder(itemCount: p.sources.length, onReorder: p.reorderSource, itemBuilder: (ctx, i) => _buildItem(p, p.sources[i])) 
                      : ListView.separated(itemCount: p.sources.length, separatorBuilder: (ctx, i) => const Divider(height: 1), itemBuilder: (ctx, i) => _buildItem(p, p.sources[i]));
  }
  ...
  Future<void> _scanQrCode(BuildContext context, SourceManagerProvider p) async {
    ...
    if (res != null && res.isNotEmpty) { final count = await p.importFromQr(res); if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('成功匯入 $count 個書源'))); }
  }

  Future<void> _importFromFile(BuildContext context) async {
    try {
      final res = await FilePicker.platform.pickFiles(type: FileType.custom, allowedExtensions: ['json', 'txt']);
      if (res?.files.single.path != null) { final count = await context.read<SourceManagerProvider>().importFromText(await File(res!.files.single.path!).readAsString()); if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('從檔案匯入 $count 個書源'))); }
    } catch (e) { if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('出錯: $e'))); }
  }

  Future<void> _importFromClipboard(BuildContext context) async {
    final data = await Clipboard.getData(Clipboard.kTextPlain);
    if (data?.text != null) { final count = await context.read<SourceManagerProvider>().importFromText(data!.text!); if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('從剪貼簿匯入 $count 個書源'))); }
  }
}
