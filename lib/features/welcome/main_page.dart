...
import 'package:legado_reader/features/association/intent_handler_service.dart';
...
class _MainPageState extends State<MainPage> with WidgetsBindingObserver {
  ...
  Future<void> _checkBackupSync() async {
    ...
    if (remote != null && mounted) {
      if (!mounted) return;
      showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text('發現新備份'), content: Text('是否從 WebDav 還原：$remote？'), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('取消')), ElevatedButton(onPressed: () async { Navigator.pop(ctx); await WebDAVService().restoreFromFile('/legado/$remote'); if (mounted) context.read<BookshelfProvider>().refreshBookshelf(); }, child: const Text('還原'))]));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<SettingsProvider>(builder: (context, settings, child) {
      final List<Map<String, dynamic>> items = [{'icon': Icons.book_outlined, 'selectedIcon': Icons.book, 'label': '書架', 'page': const BookshelfPage()}];
      ...
      items.add({'icon': Icons.source_outlined, 'selectedIcon': Icons.source, 'label': '書源', 'page': const SourceManagerPage()});
      if (settings.showRss) items.add({'icon': const Icon(Icons.rss_feed), 'label': '訂閱', 'page': const RssSourcePage()});
      items.add({'icon': Icons.settings_outlined, 'selectedIcon': Icons.settings, 'label': '設定', 'page': const SettingsPage()});
