...
class BrowserProvider extends BaseProvider {
  ...
  Future<void> init() async {
    ...
    await runTask(() async {
      final source = params.sourceOrigin != null 
          ? await BookSourceDao().getByUrl(params.sourceOrigin!) 
          : null;
  ...
  Future<void> saveVerificationResult(String currentHtml) async {
    ...
    await runTask(() async {
      if (params.refetchAfterSuccess) {
        final source = await BookSourceDao().getByUrl(params.sourceOrigin!);
        final analyzeUrl = AnalyzeUrl(
          params.url,
          source: source,
        );
  ...
  Future<void> disableSource() async {
    ...
    await runTask(() async {
      ...
      if (source != null) {
        ...
        await BookSourceDao().update(source);
      }
    });
  }
  ...
  Future<void> deleteSource() async {
    if (params.sourceOrigin == null) return;
    await runTask(() async {
      await BookSourceDao().delete(params.sourceOrigin!);
    });
  }
}
