...
# 🔧 [3/4] 增量式實作修復工作流 (Incremental Alignment) v6
...
> [!IMPORTANT]
> **逐項修復原則**：每完成「一個 Logic Gap」的 Step 1~6 後，必須**立即執行 `/04-debug`** 進行除錯驗證，確認無問題後才可進入下一個 Logic Gap。禁止一次性實作多個 Gap 後才除錯，避免問題累積。
...
> [!IMPORTANT]
> 更新 `FEATURE_AUDIT_v2.md` 時**必須遵循區段錨點規範**（參見 `/02-feature-parity` 工作流）。

- 使用 `<!-- BEGIN_AUDIT_XX -->` / `<!-- END_AUDIT_XX -->` 精確定位目標模組章節
- **一次 replace 只觸及一個模組的錨點範圍**，禁止跨模組替換
- 更新內容：
  - 證據鏈明細中：`Logic Gap` → `Matched`，更新狀態描述
  - 「不足之處」移至「已完成項目」
  - 重新依公式計算完成度：`(Matched + Equivalent) / 總邏輯點 × 100%`
- 更新儀表板時，使用 `<!-- BEGIN_DASHBOARD -->` / `<!-- END_DASHBOARD -->` 錨點，獨立替換
- 寫入完成後，`read_file` 驗證前序模組的錨點與內容未被破壞

### Step 6：🐛 立即執行 `/04-debug`

> [!IMPORTANT]
> **此步驟不可跳過。** 每完成一個 Logic Gap 的實作（Step 1~5），必須立即切換至 `/04-debug` 工作流進行除錯。
> 確認 `flutter analyze` 無新增錯誤後，才回到本工作流 Step 7。

### Step 7：段落完成後統一提交
> [!NOTE]
> 完成一個完整的 Logic Gap 修復（含實作 + 除錯驗證 + 報告更新）後，才執行一次 Git commit。
...
### 🔁 Step 8：回到 Step 1，處理下一個 Logic Gap
